...
var $reduziert_meta_fields = array(
...
register_post_type('reduziert', array(
			'labels' => array(
				'name' => __( 'Schnaeppchen' ),
				'singular_name' => __( 'schnaeppchen' ),
				'add_new' => __( 'schnaeppchen hinzufügen' ),
				'add_new_item' => __( 'schnaeppchen hinzufügen' ),
				'edit' => __( 'Bearbeiten' ),
				'edit_item' => __( 'schnaeppchen bearbeiten' ),
				'new_item' => __( 'Neues schnaeppchen' ),
				'view' => __( 'schnaeppchen ansehen' ),
				'view_item' => __( 'schnaeppchen ansehen' ),
				'search_items' => __( 'schnaeppchen suchen' ),
				'not_found' => __( 'Keine schnaeppchen gefunden' ),
				'not_found_in_trash' => __( 'Keine schnaeppchen im Papierkorb' ),
...
/*		register_post_type('angebot', array(
		...
		*/
		...
		add_filter("manage_edit-reduziert_columns",		array(&$this, "custom_columns_reduziert"));		
//		add_filter("manage_edit-angebot_columns",	array(&$this, "custom_columns_angebot"));
	...
	function custom_columns_reduziert($columns) {
		$columns = array(
			"cb"				=> "<input type=\"checkbox\" />",
			"title"				=> "Title",
			"extra_type"		=> "Typ",
	...
	function wp_insert_post($post_id, $post = null) {
		...
		switch ($post->post_type) {
			...
			case 'reduziert'	:	$post_fields = array_merge($post_fields, $this->reduziert_meta_fields);		break;			
	...
	function metabox_init() {
		...
		add_meta_box("TARG_meta", "Eigenschaften", array(&$this, "metabox_body"), "reduziert",		"normal", "low");		
	...
	function metabox_body() {
		...
		//		echo_select_for_ids("extra4_id",		"Zubehör 4",	$extras,	$custom);
			...
			if($post->post_type == 'reduziert'){
				echo_input_text('reduziert_type',			'Typ',	$custom);
			}
			...
			if($post->post_type == 'serie' || $post->post_type == 'ofen' || $post->post_type == 'paket' || $post->post_type == 'angebot'){
				?> <h4>Ofen Eigenschaften</h4><?php
				 ...
				 echo_select("dibtlabel",		"Dibt geprüft",	array("ja", "nein"),											$custom); 
				 ...
				 echo_input_text('feinstaub',	'Feinstaub',	$custom, 'mg/m³');	
				 echo_input_text('abgastemperatur',	'Mitt. Abgastemp',	$custom, '°C');
				 echo_input_text('abstandHinten',	'Abstand hinten',	$custom, 'mm');
				 echo_input_text('abstandSeite',	'Abstand seite',	$custom, 'mm');
				 echo_input_text('abgasmassenstrom',	'Abgasmassenstrom',	$custom);
				 echo_input_text('foerderdruck',	'Förderdruck P ',	$custom, 'pa');
				 echo_input_text('wirkungsgrad',	'Wirkungsgrad',	$custom, '%');
				 echo_input_text('frischluft',	'Frischluftanschluß',	$custom);
				 echo_input_text('Mehrfachbelegungstauglich',	'Mehrfachbelegungstauglich',	$custom);
				 echo_input_text('hbt',			'H/B/T',		$custom, 'cm'); 
				 echo_input_text('leistung',	'Geprüfte Leistung',		$custom, 'kW'); 
				 echo_input_text('raum_qm',		'Raumgröße',	$custom, 'm²');
				 echo_input_text('bauart',	'Bauart',	$custom); 
			}
			...
			if($post->post_type == 'ofen' || $post->post_type == 'angebot' || $post->post_type == 'paket'){
				...
				echo_textarea('eigenschaften',			'Serienmäßige Eigenschaften',	$custom);
				echo_textarea('extra_eigenschaften',	'Prüfungen',		$custom);
			}
...
function co_get_the_content($content){
	...
	if($post->post_type == 'extra' || $post->post_type == 'ofen' || $post->post_type == 'reduziert'){		
		$has_header = false;
	...
	if($post->post_type == 'paket'){
		...
		if( !empty($custom['eigenschaften']) ? ', ' : ''){
		$new_content.= "<p><span class='_983D25'>Eigenschaften: </span></br>".$custom['eigenschaften'];
		}
		
		if( !empty($custom['extra_eigenschaften']) ? ', ' : ''){
		$new_content.= "<p><span class='_983D25'>Prüfungen: </span></br>".$custom['extra_eigenschaften'];
		}
		...
		$new_content.= postmeta_with_label("dibtlabel",		"Dibt geprüft",	$custom);
		$new_content.= postmeta_with_label("brennstoff",	"Brennstoff",	$custom);
		$new_content.= postmeta_with_label("holzlaenge",	"Brennholzlänge",	$custom, "cm");
		$new_content.= postmeta_with_label("feinstaub",	    "Feinstaub",	$custom, "mg/m³");
		$new_content.= postmeta_with_label("abgastemperatur",	"Mittlere Abgastemp. ",	$custom, "°C");
		$new_content.= postmeta_with_label("abstandHinten",	"Abstand zu brennbarem </br> Material, hinten",	$custom, "mm");
		$new_content.= postmeta_with_label("abstandSeite",	"Abstand zu brennbarem Material </br>, seitlich",	$custom, "mm");
		$new_content.= postmeta_with_label("abgasmassenstrom",	"Abgasmassenstrom",	$custom);
		$new_content.= postmeta_with_label("foerderdruck",	"Mittlerer Förderdruck P ",	$custom, "pa");
		$new_content.= postmeta_with_label("wirkungsgrad",	"Wirkungsgrad",	$custom, "%");
		$new_content.= postmeta_with_label("frischluft",	"Frischluftanschluß",	$custom);
		$new_content.= postmeta_with_label("Mehrfachbelegungstauglich",	"Mehrfachbelegungstauglich",	$custom);
		$new_content.= postmeta_with_label("leistung",		"Geprüfte Leistung",		$custom, "kW");
		...
		$new_content.= postmeta_with_label("bauart",	"Bauart",	$custom);	
	...
	if($post->post_type == 'ofen'){
		...
		if(!empty($custom['serie_id'])){
			...
			if(!empty($custom['extra_empfehlung1_id']) || !empty($custom['extra_empfehlung2_id']) || !empty($custom['extra_empfehlung3_id']) || !empty($custom['extra_empfehlung4_id'])){
				$new_content.= "<h3><br><span class='_983D25'>Das passende Zubehör</span></h3>";
		...
		$new_content.= postmeta_with_label("abgastemperatur",	"Mittlere Abgastemp. ",	$custom, "°C");
		$new_content.= postmeta_with_label("abstandHinten",	"Abstand zu brennbarem </br> Material, hinten",	$custom, "mm");
		$new_content.= postmeta_with_label("abstandSeite",	"Abstand zu brennbarem </br> Material, seitlich",	$custom, "mm");
		$new_content.= postmeta_with_label("abgasmassenstrom",	"Abgasmassenstrom",	$custom);
		$new_content.= postmeta_with_label("foerderdruck",	"Mittlerer Förderdruck P ",	$custom, "pa");
		$new_content.= postmeta_with_label("wirkungsgrad",	"Wirkungsgrad",	$custom, "%");
		$new_content.= postmeta_with_label("frischluft",	"Frischluftanschluß",	$custom);
		...
		$PreisEuro= $custom["preiss"]."<a href='http://gussco.de/angebot-einholen/'> Angebot einholen </a>";
		$new_content.= postmeta_with_label_class("Preis",	$PreisEuro	 , "_bold_black", "");
