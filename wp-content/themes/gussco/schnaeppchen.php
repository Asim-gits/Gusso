	...
	<div id="main4">
		<div id="container4">
			<div id="content4">
				...
				function sort_reduzierts($a, $b){
				    if(isset($a->custom['reduziert_type']) && isset($a->custom['reduziert_type'])){
				    	if($a->custom['reduziert_type'] == $b->custom['reduziert_type']){
				    		return $a->post_title > $b->post_title;
				    	}
				    	
				    	return $a->custom['reduziert_type'] > $b->custom['reduziert_type'];
				    }
				...
				switch ($post->post_title) {
					...
					break;
					case "Ausstellungsstücke":
						?>
						<div id="special_content">
							<h1><span class="_983D25">Kaminöfen</span><span class="_000000">-aus unserer Ausstellung</span></h1>
<font size="4" face="Georgia"><i>Hier finden Sie unsere reduzierten Kamine und Öfen laut UVP.</i></span></font>
<p></p>
<font size="4" face="Georgia"><i>Nur Abholung oder Lieferung & Montage unsererseits möglich. Kein Versand, da keine Verpackungseinheiten vorhanden sind.</i></span></font>
			...
			for(var i = 0; i < ovens.length; i++){
				...
				if(others){			
					...
					if(others == 'Speckstein')	{
						...
						if(oven.custom.hasOwnProperty("reduziert_eigenschaften")	&& oven.custom.reduziert_eigenschaften.search("Speckstein") != -1)	hasSpeckstein = true;
					...
					if(others == 'seitliches Glasfenster')	{
						...
						if(oven.custom.hasOwnProperty("reduziert_eigenschaften")	&& oven.custom.reduziert_eigenschaften.search("seitliches Glasfenster") != -1)	hasFenster = true;
				...
				if(!isHidden) {
					...
					case "Ausstellungsstücke" :
							if(!empty($reduzierts)){
								...
								for ($i = 0; $i < count($reduzierts); $i++){
									$custom = get_post_custom($reduzierts[$i]->ID);
									
									foreach($custom as $key => $value)
										if(is_array($value) && count($value) == 1) $custom[$key] = $value[0];
										
									$reduzierts[$i]->custom = $custom;
								}
								
								usort($reduzierts, "sort_reduzierts");
							
								foreach ($reduzierts as $reduziert){
								?><div class='div_product_thumb_overview' id="<?php echo "reduziert_$reduziert->ID" ?>">
									<a href="<?php echo post_permalink($reduziert->ID); ?>"><span class="gray">
										<?php echo get_the_post_thumbnail($reduziert->ID, array(52,85)); ?>
										<div class="with_spacing_before">
											<span style="font-family:Georgia; font-size:16px; color:#000;"><?php echo preg_replace("/([0-9]+(.+)?)/", "<span style='color:#983D25'>$0</span>", $reduziert->post_title); ?></span>
											<?php
												echo postmeta_with_label("preiss",		"Preis",		$reduziert->custom, "€");
											?>
										</div>
									</span></a>
									</div><?php 
